...
void pluginMessage(const char *msg) {
  int pluginFifo = open(
      PLUGIN_FIFO,
      O_WRONLY |
          O_NDELAY);    // In case we have a fingerprint-plugin running, stop it
...
bool requestFingerprint(int pipe_w, const char *display, char *service,
  ...
  if (devices == nullptr) {
    mode = MODE_VERIFY;
    syslog(LOG_WARNING,
           "WARNING: No devices can identify. Fallback to verifying only.");
  ...
  if (username == nullptr) {     // User not yet known, maybe it's a login?
    if (mode == MODE_IDENTIFY) { // Device can identify, so identifying the user
      ...
      for (; devices != nullptr; devices = devices->next) {
        syslog(LOG_DEBUG, "Looking for fingerprints for device %s.",
      ...
      if (lastResult >= 0) {
        ...
        userSent = true;
        if (write(pipe_w, user, strlen(user) + 1) <=
            0) { // Writing user to fifo
          syslog(LOG_DEBUG, "ERROR: Writing user to pipe (%s).",
                 strerror(errno));
          return false;
        }
  ...
  for (; devices != nullptr; devices = devices->next) {
    syslog(LOG_DEBUG, "Looking for fingerprints for user %s with device %s.",
  ...
  if (mode == MODE_VERIFY) {
    ...
    devices->setIdentifyData(identifyData);
    syslog(LOG_DEBUG, "Identifying all discovered fingerprints from %s.",
           username);
  }
  ...
  if (display == nullptr
    ...
    syslog(LOG_INFO, "Have X-display %s. Starting GUI authentication.",
           display);
    QApplication app(argc, argv);
    loadTranslations(app);
...
int main(int argc, char **argv) {
  ...
  if (argc < 4) {
    syslog(LOG_ERR, "Wrong number of arguments.\nIt's not intended to start "
                    "fingerprint-helper manually.\n");
    return (EXIT_FAILURE);
  }

  signal(SIGUSR1, handler_SIGUSR); // Install a handler for SIGUSR1: Exit when
                                   // we didn't send a username before
  signal(SIGUSR2,
         handler_SIGUSR); // Install a handler for SIGUSR2: Exit immediately

  sscanf(
      argv[1], "%d",
      &pipe_r); // Get the pipe handle for reading from pam_fingerprint-gui.so
  sscanf(argv[2], "%d",
  ...
  // Try to get a saved password from external media (USB stick) only when it
  // runs as root (PAM)
  ...
  if (geteuid() == 0) { // running as root
    ...
    if (userSettings.getPassword(&savedPassword)) {
      ...
      //!!!!!!!!!!!!!!!! USE THIS LOG OUTPUT ONLY FOR DEVELOPMENT
      //!!!!!!!!!!!!!!!!!!!!!
      if (write(pipe_w, savedPassword, strlen(savedPassword) + 1) <=
          0) { // Writing this password to fifo
        syslog(LOG_DEBUG, "ERROR: Writing password (%s) to pipe.",
               strerror(errno));
        free(savedPassword);
        pluginMessage(MSG_STOP);
        closelog();
  ...
  if (havePassword != true) {
    syslog(LOG_INFO, "Have no password from external media."); // Writing random
                                                               // string to fifo
    if (write(pipe_w, randomString, sizeof(randomString)) <= 0) {
      syslog(LOG_DEBUG, "ERROR: Writing random number (%s) to pipe.",
             strerror(errno));
      pluginMessage(MSG_STOP);
      closelog();
      unlink(HELPER_PID);
