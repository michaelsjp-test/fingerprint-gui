...
UpekDevice::UpekDevice(void *libHandle, struct abs_device_list_item *bs,
                       USBDevice *knownUSBDevices)
    : FingerprintDevice() {
  char *error;
  bsData = nullptr;
  conn = 0;
  ...
  *(void **)(&bsapiPropertyFunction) = dlsym(bsapiHandle, BSAPI_PROPERTY);
  if ((error = dlerror()) != nullptr) {
    syslog(LOG_ERR, "Could not find symbol \"%s\" (%s).", BSAPI_PROPERTY,
           error);
  }
  ...
  *(void **)(&bsapiTerminateFunction) = dlsym(bsapiHandle, BSAPI_TERMINATE);
  ...
  if (!(s.find("VID_", 0) == string::npos ||
        s.find("_PID_", 0) == string::npos)) {
    vendorId = strtol(s.substr(s.find("VID_", 0) + 4, 4).data(), nullptr, 16);
    productId = strtol(s.substr(s.find("_PID_", 0) + 5, 4).data(), nullptr, 16);
    syslog(LOG_INFO, "initializing libbsapi device (vend/prod) 0x%x/0x%x",
           vendorId, productId);
  ...
  if (s.find("device=#", 0) != string::npos) { // Found for 147e:1000 devices
    vendorId =
        strtol(s.substr(s.find("device=#", 0) + 10, 4).data(), nullptr, 16);
    productId =
        strtol(s.substr(s.find("device=#", 0) + 15, 4).data(), nullptr, 16);
    syslog(LOG_INFO, "initializing libbsapi device (vend/prod) 0x%x/0x%x",
           vendorId, productId);
    if (knownUSBDevices != nullptr) {
      for (USBDevice *u = knownUSBDevices; u != nullptr; u = u->next) {
  ...
  if (s.find("device=", 0) != string::npos) { // Found for 147e:2016 devices
    vendorId =
        strtol(s.substr(s.find("device=", 0) + 7, 4).data(), nullptr, 16);
    productId =
        strtol(s.substr(s.find("device=", 0) + 12, 4).data(), nullptr, 16);
    syslog(LOG_INFO, "initializing libbsapi device (vend/prod) 0x%x/0x%x",
           vendorId, productId);
    if (knownUSBDevices != nullptr) {
      for (USBDevice *u = knownUSBDevices; u != nullptr; u = u->next) {
  ...
  syslog(LOG_ERR, "FIXME! Couldn't parse vendor/productin line:\"%s\"",
         s.data());
}
...
void UpekDevice::setMode(int m) { mode = m; }
...
void UpekDevice::setData(void *d, int) { bsData = (ABS_BIR *)d; }
...
void UpekDevice::stop() {
  ...
  if (bsapiSetledFunction)
    (*bsapiSetledFunction)(conn, ABS_LED_MODE_OFF, 0,
                           0); // lED would stay on after cancelling
  if (bsapiCancelFunction)
  ...
  if (bsapiOpenFunction) {
    ...
    if (res != ABS_STATUS_OK) {
      ...
      if (bsapiErrorFunction) {
        ...
        syslog(LOG_ERR, "ABSOpen() failed %d (%s).", (int)ErrorCode,
               ErrorMessage);
        return false;
      }
      return false;
          ...
          if (!nvmPath.isEmpty()) { // We have an NVM path entry
            ...
            if (var.exists()) { // A directory for NVM data exists
              ...
              foreach (QString entry,
                       var.entryList(QDir::Hidden | QDir::Files)) {
                QFile f(nvmPath + "/" + entry); // This is the NVM file
                syslog(LOG_DEBUG, "NVM file \"%s\" detected.",
                       f.fileName().toStdString().data());
                f.setPermissions(QFile::ReadUser | QFile::WriteUser |
                                 QFile::ReadGroup | QFile::WriteGroup |
                                 QFile::ReadOther | QFile::WriteOther);
                syslog(LOG_DEBUG, "NVM file permissions 0%x.",
                       0xfff & (int)f.permissions());
              }
            }
          }
        }
...
void BSAPI callback(const ABS_OPERATION *p_operation, ABS_DWORD msg,
                    void * /*data*/) {
  switch (msg) {
  ...
  case ABS_MSG_PROMPT_SCAN:
    if (lastMsg == ABS_MSG_PROCESS_SUCCESS &&
        p_operation->OperationID == MODE_ACQUIRE) {
...
bool UpekDevice::verify() {
  ...
  if ((*bsapiVerifyFunction)(conn, &operation, 1, &bsData, &bsResult, 0) !=
      ABS_STATUS_OK) {
...
bool UpekDevice::identify() {
  ...
  if (bsapiVerifyFunction) {
    if ((*bsapiVerifyFunction)(conn, &operation, numIdentify, identifyData,
                               (ABS_LONG *)&match, absFlag) != ABS_STATUS_OK) {
